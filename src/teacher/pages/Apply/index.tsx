...
function Apply({ me }: Props) {
	...
	useEffect(() => {
		(async () => {
			...
			const description = items.find((item) => item.key === 'teacher/application/description')!.value;
	...
	if (application.id !== 0 && application.status === ApplicationStatus.STATUS_COMMIT) {
		alert = (
			<Alert style={{ marginBottom: '1em' }} message="申请已提交" description="您的申请正在审核中，请耐心等待，后续通知将通过注册邮箱发送。" type="info" />
		);
	} else if (application.status === ApplicationStatus.STATUS_SUCCESS) {
		alert = <Alert style={{ marginBottom: '1em' }} message="申请已通过" description="您的申请已通过！" type="success" />;
